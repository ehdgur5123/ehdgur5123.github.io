...
```python
import re
text = ['file01.txt','file02.pdf','@**@30.txt']

for i in text:
  result = re.search(r'\w+(?=\.txt)',i)
  print(i, ': ', result)
```

`\w+(?=\.txt)`은 뒤에 `.txt`가 있을 경우 '단어 문자'(`\w`)가 '한 번 이상 반복되는 것'(`+`)을 매칭

```
file01.txt :  <re.Match object; span=(0, 6), match='file01'>
file02.pdf :  None
@**@30.txt :  <re.Match object; span=(4, 6), match='30'>
```

- `file01.txt`는 뒤에 `.txt`가 있으므로 단어 문자인 `file01`을 매칭
- `file02.pdf`는 뒤에 `.txt`가 없으므로 `None`
- `@**@30.txt`는 뒤에 `.txt`가 있으므로 그 앞의 단어 문자에 해당하는 `30`만 매칭

### 부정 전방 탐색(패턴1(?!패턴2))


5. 부정 전방 탐색 (Negative Lookahead): (?!...)
정의: 특정 패턴이 뒤에 오지 않을 경우에만 매칭.
예시: \d+(?! apples)는 숫자 뒤에 "apples"가 없는 경우 매칭.
추가로 자주 사용되는 PCRE 기능:
6. 후방 탐색 (Lookbehind): (?<=...)
정의: 특정 패턴이 앞에 올 경우에만 매칭.
예시: (?<=\$)\d+는 $ 기호 뒤에 나오는 숫자와 매칭.
7. 부정 후방 탐색 (Negative Lookbehind): (?<!...)
정의: 특정 패턴이 앞에 오지 않을 경우에만 매칭.
예시: (?<!\$)\d+는 $ 기호가 없는 숫자를 매칭.
8. | (OR 연산자)
정의: 여러 패턴 중 하나와 일치할 때 사용.
예시: apple|banana는 "apple" 또는 "banana"와 일치.
9. 그룹 캡처 및 참조: ( )
